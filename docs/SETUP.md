...
**절차** (`test_square.py --camera 1` 안에서):

```
# 판의 서로 다른 구석 3~4곳에서 반복
square> a5              ← 그 칸으로 이동
square> ...             ← 흡착컵 끝이 칸 중심에 '정확히' 오도록 눈으로 맞춤
square> markcal add a5  ← 기록

square> h5
square> markcal add h5
square> a8
square> markcal add a8
square> h8
square> markcal add h8

square> markcal fit     ← 계산 + colors.json 저장 (바로 적용)
```

```
4점으로 계산: 잔차 평균 0.8mm, 최대 1.4mm
저장 완료 → ...\vision\colors.json (바로 적용됨)
```

- **점들이 한 줄로 늘어서면 안 된다** — 판을 넓게 덮도록 고를 것
- 잔차가 8mm 넘게 나오면 흡착컵을 칸 중심에 제대로 안 맞춘 것
- `markcal clear` 로 초기화, `markcal <칸>` 은 한 칸만 보는 간이 방식
