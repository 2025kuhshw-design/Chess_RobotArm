...
| `set 38 140 180` | 현재 위치 가정만 교정 (전송 안 함) |
| `home` | PARK 자세로 복귀 |
| `q` | 종료 (팔은 현재 위치 유지) |
...
- 보드가 **로봇 왼쪽으로 5.7cm 치우친다.** 베이스가 `s1=38`이라 오른쪽 회전
  여유가 38°뿐이어서, 왼쪽으로 옮겨야 전체가 가동범위에 들어온다.
