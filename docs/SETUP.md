...
## 11-A. 카메라 캘리브레이션 — 자동 ⭐ 권장

사람이 네 모서리를 클릭하는 방식은 두 가지 한계가 있다.
- 손으로 찍으니 몇 px 어긋나고, 그 오차가 판 전체로 퍼진다
- 네 점만 쓰므로 클릭 실수를 걸러낼 방법이 없다
- 게다가 판에 **딱 맞춰 찍으면 여유가 없어** 가장자리에서 마커가 잘린다

`findChessboardCorners` 로 **내부 교점 49개(7×7)** 를 한꺼번에 찾고,
그 49점에 격자를 최소제곱으로 맞춰 바깥 모서리를 계산한다.
...
| 방식 | 모서리 오차 |
|---|---|
| **자동 (49점 최소제곱)** | **0.76px** (칸의 1.3%) |
| 수동 클릭 (손떨림 ±3px 가정) | 3.71px |

**되는 조건** — 안 되면 수동으로 돌아가면 된다:
- 체스판에 **기물이 하나도 없어야** 한다 (기물이 교점을 가린다)
- 판 전체가 화면에 들어와야 한다
- 밝고 고른 조명. 강한 그림자·반사가 있으면 실패한다
- 칸 대비가 뚜렷해야 한다 (아주 옅은 나무색은 실패할 수 있다)

실패하면: `python vision/calibrate.py --camera 1` (수동)

> 잔차가 5px 넘게 나오면 경고가 뜬다 — 렌즈 왜곡이 심하거나 일부 교점을
> 잘못 잡은 것이다.

### 판 바깥 여유는 자동으로 확보된다

탑뷰는 판보다 **한 칸 반 더 넓게** 편다(`TOP_MARGIN_CELLS = 1.5`).
그래서 a·h 파일이나 8랭크처럼 가장자리 칸에서도 흡착컵 마커가 화면 안에 남는다.
캘리브레이션은 판 모서리만 지정하면 되고, 여유는 코드가 알아서 붙인다.
...
#### ⭐ 그 전에 — 카메라 노출·화이트밸런스부터 고정한다

```bash
python vision/lock_camera.py --camera 1
```

이걸 건너뛰면 **색을 아무리 잘 골라도 소용없다.** 실제로 겪은 일:

- `check_board --watch` 는 61 → 62 → 64/64 로 맞아 가는데, `--watch` 없이
  한 장만 읽으면 48/64 였다 → 자동노출·AWB 가 수렴하는 데 몇 초 걸린다.
  라이브 뷰는 수렴 후를 보고, 방금 연 프로그램은 수렴 전을 본다.
- 감마를 3.0 까지 올려도 밝은 칸이 흰 기물 범위에 들어왔다.
  출력값을 역산하니 밝은 칸의 **채도가 3** 이었다(나무색이면 80 근처).
  자동 화이트밸런스가 화면 평균을 무채색으로 맞추려는데 이 판은 대부분이
  베이지색이라, 그 색조를 통째로 상쇄해 **밝은 칸을 흰색으로 만든 것**이다.
  채도가 0에 가까우면 흰 기물과 같은 색이라 원리상 구분이 불가능하고,
  감마로도 못 살린다(0에 뭘 곱해도 0이다).

카메라가 OpenCV 로 고정이 안 되는 기종이면:
...
#  화면 위 sat light 가 60 이상인지 먼저 확인 (20 아래면 lock_camera 부터)
#  b 누르고 → 시작 배치의 빈 칸 32개를 음성 표본으로 자동 수집  ★
#  1 누르고 → 흰쪽 기물 윗면 클릭 (여러 개)
#  2 누르고 → 검은쪽 기물 윗면 클릭
#  s 저장 → vision/colors.json (색 방식이 자동으로 켜진다)
```

화면 아래에 `cells white=16 black=16` 이 뜨고 `board leak` 이 없으면 성공.
기물 윗면만 칠해지고 판은 안 칠해져야 한다.

**음성 표본(b)을 꼭 모을 것.** 예전에는 표본 범위를 무조건 S±70 으로 넓혀서
흰 기물 범위가 카페라떼 칸을 삼켰다(재현 결과 48/64, 판 픽셀 219/800 오검출).
지금은 음성이 안 들어가는 선까지만 넓히므로 같은 조건에서 64/64 가 된다.
...
### ⭐⭐ 시차 보정 (markcal) — 집기 정확도의 마지막 단계

**증상**: 정렬이 "완료"라고 하고 화면상 마커도 칸 중앙인데, **흡착컵은 실제로
벗어나 있다.**

**원인**: 정렬이 맞추는 건 '마커'다. 그런데 카메라가 완전한 수직이 아니면
마커는 시차로 밀려 보이므로, **마커를 중심에 맞추면 흡착컵은 반대편으로 밀린다.**

| 판 중심에서의 거리 | 흡착컵 실제 오차 (측정 높이 25mm 기준) |
|---|---|
| 5cm | 3.3mm |
| 10cm | 6.7mm |
| 15cm | 10.0mm |

밀림이 **위치에 비례**하므로 상수 하나로는 못 잡는다. 여러 칸에서 재서
어파인 변환을 구해야 한다.

**절차 — 자동** (`test_square.py --camera 1` 안에서). 손으로 옮길 필요 없다:

```
square> markcal auto    ← 판 위 기물을 치우고 실행. 팔이 20칸을 돌며 잰다 (약 80초)
square> markcal fit     ← 계산 + colors.json 저장 (바로 적용)
```

**왜 명령한 칸을 '정답'으로 써도 되나**: 없애려는 시차는 **위치에 비례하는
계통 오차**이고, 관절 유격은 **매번 방향이 달라지는 무작위 오차**다.
여러 점을 최소제곱하면 무작위는 상쇄되고 계통 성분만 남는다.
그래서 팔이 부정확해도 시차는 제대로 뽑힌다 — 점이 많을수록 잘 뽑힌다.

모의실험 (유격 ±5mm 가정, 도달 가능한 칸 전체 평균):

| | 보정 후 평균 오차 |
|---|---|
| 보정 안 함 | 4.9mm (최대 9.0mm) |
| 6칸 | 4.0mm |
| 12칸 | 3.2mm |
| **20칸 (기본)** | **2.5mm** |

> 유격이 ±2mm 로 줄면 20칸에서 **1.0mm** 까지 간다. 즉 베이스 관절을
> 기계적으로 조이는 게 여전히 가장 큰 개선 요인이다.

**수동 보정** (더 정확하게 하고 싶을 때). `nudge` 로 흡착컵을 칸 중심에 맞춘다:

```
square> a5
square> nudge 2 -3       ← 파일 +2mm, 랭크 -3mm 이동 (눈으로 보며 반복)
square> markcal add a5   ← 지금 위치를 'a5 중심'으로 기록
... 3칸 이상 반복 후
square> markcal fit
```

- 점들이 **한 줄로 늘어서면 안 된다** — 판을 넓게 덮도록
- `markcal clear` 초기화 / `markcal <칸>` 한 칸만 보는 간이 방식

> 카메라나 판을 옮기면 다시 해야 한다.
...
- **RL 보정은 기본으로 꺼져 있다.** 켜려면 `--rl`.
  카메라 폐루프 보정은 *실제로 잰* 오차를 고치는 반면, RL 보정은 재지 않은
  값을 얹는 것이라 둘을 같이 쓰면 오히려 어긋난다.
  (실측: RL을 켜니 e7e5가 e파일 근처도 못 가고 f8을 한 칸 빗나갔다)
- 카메라 화면은 **기본으로 켜진다.** 끄려면 `--no-view`.
