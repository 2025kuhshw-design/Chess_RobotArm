...
TOUCH_PRESS      = 0.013       # 13mm 더 눌러 내려감
                               # (기물 4~5mm 기준. 너무 키우면 판을 긁는다)
LIFT_HEIGHT      = 0.04        # 기본 안전 높이 (m) — 기물 7mm 기준 충분
LIFT_MIN         = 0.015       # 최소 리프트 (m) — 기물(7mm)보다 커야 함

# ─────────────────────────────────────────
# 하강 중 생기는 밀림의 추정치 (칸 단위, [파일, 랭크])
# ─────────────────────────────────────────
# ⚠️ 정렬은 측정 높이(기물 위 20.5mm)에서 끝난다. 거기서 누르는 높이까지
#    내려가는 동안 팔이 옆으로 밀린다 — "정렬은 중앙이었는데 내리니까
#    벗어난다"는 증상이 이것이다.
#    관절보간 탓은 아니다(모형상 수평 이탈 최대 1.1mm, 실측 시뮬레이션).
#    중력 처짐·유격·흡착컵 고무 변형이라 계산으로는 못 잡는다.
#    → 놓은 기물이 칸 중심에서 얼마나 벗어났는지 카메라로 재서, 그만큼을
#      다음 하강에 미리 빼준다. hardware/verify_move.py 가 갱신한다.
DESCENT_BIAS = [0.0, 0.0]
...
class RealArm:
    ...
    def execute_move(self, from_sq: tuple, to_sq: tuple,
        ...
        def _move_to(col, row, lift=False, suction=False, use_offset=True):
            ...
            dc, dr = (off[0], off[1]) if use_offset else (0.0, 0.0)
            if not lift:
                # 하강 중 밀리는 만큼을 미리 반대로 빼둔다.
                # ⚠️ 안전높이 이동에는 걸지 않는다 — 밀림은 '내려가는 동안'
                #    생기는 것이라 위에서 미리 빼두면 정렬이 그걸 또 지운다.
                dc -= DESCENT_BIAS[0]
                dr -= DESCENT_BIAS[1]
            if dc or dr:
